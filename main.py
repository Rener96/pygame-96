...
class Player:
    ...
    def move(self, obstacles, enemies):
        ...
        for enemy in enemies:
            if self.rect.colliderect(enemy.rect):
                self.respawn()
                return "death"
        if self.speed > 5 and self.move_dir != 0:
            self.ghosts.append({'x': self.rect.x, 'y': self.rect.y, 'alpha': 150, 'img': self.player})
    ...
    def draw(self, camera_x, camera_y):
        ...
        for ghost in self.ghosts:
            ghost_surface = ghost['img'].copy()
            ghost_surface.set_alpha(ghost['alpha'])
            screen.blit(ghost_surface, (ghost['x'] - camera_x, ghost['y'] - camera_y))
...
def start_the_game():
    ...
    while running:
            ...
            if event.type == pygame.KEYUP and not game_over:
                ...
                if event.key == pygame.K_a or event.unicode in ('ф', 'Ф'):
                    if player.move_dir == -1:
                        player.move_dir = 0
        if not game_over:
            res = player.move(obstacles, enemies)
            if res == "death":
                deaths += 1
            for enemy in enemies:
                enemy.update()
        ...
        bg_x = 40 - camera_x
        bg_y = -1000 - camera_y
        pygame.draw.rect(screen, (40, 40, 40), (bg_x, bg_y, 720, 1900))
        for obstacle in obstacles:
            screen_x = obstacle.rect.x - camera_x
            screen_y = obstacle.rect.y - camera_y
            if obstacle.type == "финиш":
                bobbing = (pygame.time.get_ticks() // 200)
                y_offset = int((bobbing % 4) - 2)
                screen.blit(obstacle.image, (screen_x, screen_y + y_offset))
            else:
        ...
        if not game_over:
            ...
            time_text = font_ui.render(f"Время: {current_time:.1f} сек", True, (255, 255, 255))
            deaths_text = font_ui.render(f"Смерти: {deaths}", True, (255, 100, 100))
            screen.blit(time_text, (50, 20))
            screen.blit(deaths_text, (50, 50))
        if game_over:
            text_surface = font.render("Ты прошёл!", True, (255, 255, 255))
            text_rect = text_surface.get_rect(center=(400, 200))
            screen.blit(text_surface, text_rect)
            stats_str = f"Время: {final_time:.2f} сек. | Смертей: {deaths}"
            stats_surface = font_ui.render(stats_str, True, (255, 215, 0))
            stats_rect = stats_surface.get_rect(center=(400, 280))
            screen.blit(stats_surface, stats_rect)
            hint_surface = font_ui.render("Нажми R для перезапуска игры и сброса таймера или ESC для меню", True, (200, 200, 200))
            hint_rect = hint_surface.get_rect(center=(400, 360))
            screen.blit(hint_surface, hint_rect)
        pygame.display.flip()
theme = pygame_menu.themes.THEME_DARK.copy()
theme.title_font = "arial"
theme.widget_font = "arial"
menu = pygame_menu.Menu(title='ПЛАТФОРМЕР', width=800, height=600, theme=theme)
